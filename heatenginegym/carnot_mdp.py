...
class CarnotEnv(HeatEngineEnv):
    def __init__(self, max_episode_steps=200, *args, **kwargs):
        ...
        self.observation_space = gym.spaces.Box(low=np.array([self.engine.Tmin-1e-6, self.engine.Vmin-1e-6, 0.0-1e-6, 0.0-1e-6]), high=np.array([self.engine.Tmax+1e-6, self.engine.Vmax+1e-6, 0.7824495820293804+1e-6, 0.3129798328117521+self.Wi+1e-6]),dtype=np.float32)
