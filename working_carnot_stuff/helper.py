...
def evaluate_policy(policy):
    ...
    r = np.zeros(1000)
    for i in range(1):
        action_list = np.zeros(policy.num_actions, dtype=np.int32)
        s1 = env.reset()
        s = s1
        d = False
        j = 0
        while not d:
            ...
            s = s1
            a = policy.evaluate(s)
            s1, r[j], d, _ = env.step(a)
            action_list[int(a)] = 1

        vara = np.mean(action_list)
        #if vara < 0.95:
        #    r += -0.5
        #r += vara - 1.0
        reward += np.max(r) + vara
        if (s1[0] == s[0] and s1[1] == s[1]) or (s1[0] == s0[0] and s1[1] == s0[1]):
            reward = -1.0
...
def vis_policy(policy):
    ...
    s = env.reset()
    #env.render()
    print 111.34970437, 0.001, -1.0, -1.0
    d = False
    while not d:
        a = policy.evaluate(s)
        s, r, d, _ = env.step(a)
